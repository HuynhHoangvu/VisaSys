...
export const moveTask = async (req, res) => {
    try {
        ...
        const oldTask = await prisma.task.findUnique({ where: { id: id } });
        if (!oldTask)
            return res.status(404).json({ error: "Không tìm thấy thẻ khách hàng" });
...
export const moveProcessingTask = async (req, res) => {
    try {
        const { id } = req.params;
