...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, isOpen = false, onClose }) => {
  const [workspaces, setWorkspaces] = useState<Workspace[]>([{ id: "ws-1", name: "Fly Visa" }]);
  const [activeWorkspaceId, setActiveWorkspaceId] = useState<string>("ws-1");

  const handleAddWorkspace = () => {
    const newName = window.prompt("Nhập tên Doanh nghiệp / Group mới (VD: FlyLabour):");
  ...
  const isBoss =
    ...
    ["giám đốc", "phó giám đốc"].some((r) => currentUser.role?.toLowerCase().includes(r));
  const isManager = ["quản lý", "trưởng phòng"].some((r) =>
    currentUser.role?.toLowerCase().includes(r),
  );
  const isProcessingDeptUser = ["xử lý hồ sơ", "hồ sơ", "trợ lý giám đốc"].some((d) =>
      ...
      >
        ...
        <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto custom-scrollbar">
          ...
          <NavLink to="/dashboard" className={navLinkClass} onClick={handleNavClick}>
            <svg className="w-4.5 h-4.5 mr-3 shrink-0" style={{width:"18px",height:"18px"}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {canViewBossReport && (
            <NavLink to="/boss" className={navLinkClass} onClick={handleNavClick}>
              <svg className="mr-3 shrink-0 text-red-400" style={{width:"18px",height:"18px"}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <NavLink to="/crm" className={navLinkClass} onClick={handleNavClick}>
            <svg className="mr-3 shrink-0" style={{width:"18px",height:"18px"}} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="px-3 py-3 border-t border-gray-700/50 space-y-0.5">
          ...
          <div className="flex items-center gap-2.5 px-3 py-2 mt-1 rounded-lg bg-gray-800/60">
            <div className="w-7 h-7 rounded-full bg-orange-500 text-white text-xs font-bold flex items-center justify-center uppercase shrink-0">
