...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        // Ưu tiên matching theo ID đã lưu trước, sau đó mới tới URL
        const nextActive = matchedBySaved?.id || matchedByUrl?.id || list[0].id;
  ...
  const handleSaveEdit = async () => {
    ...
    setWsSaving(true);
    try {
      const res = await api.put<Workspace>(`/api/workspaces/${editingWs.id}`, {
        name, url: wsFormUrl.trim() || undefined, employeeId: currentUser.id,
      });
      setWorkspaces((prev) => prev.map((w) => (w.id === editingWs.id ? res.data : w)));
      setWsModal("manage"); // Quay lại trang quản lý
      setEditingWs(null);
    } catch (e) {
  ...
  const handleDelete = async (wsId: string) => {
    ...
    if (!window.confirm("Bạn có chắc chắn muốn xoá không gian này?")) return;
      ...
      >
              ...
              >
                {workspaces.map((ws) => (
                  <option key={ws.id} value={ws.id}>
                    {ws.id === activeWorkspaceId ? "● " : ""}{ws.url ? `↗ ${ws.name}` : ws.name}
                  </option>
                ))}
              </select>
              {!isTeacherDeptUser && (
        ...
        {wsModal && (
          <div className="absolute inset-0 z-[60] flex items-start justify-center bg-black/60 backdrop-blur-sm pt-20">
            <div className="w-[calc(100%-24px)] bg-gray-800 rounded-xl shadow-2xl border border-gray-700 p-4 max-h-[70vh] flex flex-col">
              ...
              {wsModal === "manage" ? (
                <div className="space-y-2 overflow-y-auto pr-1 flex-1 custom-scrollbar">
                  {workspaces.map((ws) => (
                    <div key={ws.id} className={`flex items-center justify-between p-2 rounded-lg border ${ws.id === activeWorkspaceId ? "bg-orange-500/10 border-orange-500/50" : "bg-gray-700/30 border-gray-600/50"}`}>
                      <div className="min-w-0 flex-1 mr-2">
                        <div className="flex items-center gap-1.5">
                          {ws.id === activeWorkspaceId && <span className="w-1.5 h-1.5 rounded-full bg-orange-500" title="Hiện tại" />}
                          <p className="text-xs font-medium text-gray-200 truncate">{ws.name}</p>
                        </div>
                        {ws.url && <p className="text-[10px] text-gray-500 truncate">{ws.url}</p>}
                      </div>
                      <div className="flex items-center gap-1">
                        <button onClick={() => startEdit(ws)} className="p-1.5 text-gray-400 hover:text-blue-400 hover:bg-blue-400/10 rounded transition-colors">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                          </svg>
                        </button>
                        <button onClick={() => handleDelete(ws.id)} disabled={wsSaving} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-400/10 rounded transition-colors">
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button onClick={openAddModal} className="w-full py-2 px-3 flex items-center justify-center gap-2 border border-dashed border-gray-600 hover:border-orange-500 text-gray-400 hover:text-orange-500 rounded-lg transition-all text-xs">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                    </svg>
                    Thêm không gian mới
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[11px] text-gray-400 mb-1 block">Tên không gian</label>
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    <button onClick={wsModal === "add" ? handleSaveAdd : handleSaveEdit} disabled={wsSaving} className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white text-sm font-medium py-2 rounded-lg transition-colors">
                      {wsSaving ? "Đang lưu..." : wsModal === "add" ? "Thêm" : "Lưu thay đổi"}
                    </button>
                    <button onClick={wsModal === "edit" ? () => setWsModal("manage") : closeModal} disabled={wsSaving} className="flex-1 bg-gray-700 hover:bg-gray-600 disabled:opacity-50 text-gray-300 text-sm font-medium py-2 rounded-lg transition-colors">
