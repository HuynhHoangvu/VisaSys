...
const DocumentDashboard: React.FC<DocumentDashboardProps> = ({
  ...
  // Lắng nghe Socket (Đã sửa lỗi Cảnh báo Linter)
  useEffect(() => {
    ...
    socket.on("docs_changed", handleDataChange);

    return () => {
      socket.off("docs_changed", handleDataChange);
    };
  }, [fetchData]);
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchData(); // Có thể bỏ nếu đã có Socket gánh
    } catch (error) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      // Chuẩn bị form data để gửi file lên server
      const formData = new FormData();
      formData.append("file", file);
      formData.append("uploadedBy", currentUser?.name || "Admin");
      formData.append("size", formatFileSize(file.size)); // Dùng hàm format
      if (currentFolderId) {
  ...
  const handleDownload = async (
    ...
    try {
      ...
      // Chuyển dữ liệu thành dạng Blob (Binary Large Object)
      const blob = await response.blob();

      // Tạo một URL tạm thời cho Blob này
      const url = window.URL.createObjectURL(blob);

      // Tạo thẻ <a> ảo, gán link và giả lập cú click để tải về
          ...
          {displayFiles.map((file) => (
            ...
            >
              ...
              <div className="mt-auto flex justify-between items-center pt-3 border-t border-gray-100">
                ...
                {/* ĐÃ ĐỔI THÀNH BUTTON ĐỂ GỌI HÀM ÉP TẢI FILE */}
                <button
