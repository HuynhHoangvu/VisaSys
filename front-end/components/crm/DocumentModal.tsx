    ...
    const fetchLatestData = async () => {
      try {
        ...
        if (currentTask) {
          ...
          // ✅ TỰ ĐỘNG CHUYỂN CHECKLIST THEO VISA TYPE
          if (currentTask.checklistType) {
            // Ưu tiên dùng checklistType đã lưu trong DB
            setChecklistType(currentTask.checklistType);
          } else {
            // Tự detect theo tên visa nếu chưa có
           ...
           <table className="w-full text-sm text-left text-gray-600">
             ...
             <tbody className="divide-y divide-gray-200">
               {sections.map((sectionName) => {
                 // ID riêng cho hàng "Hồ sơ khác" của mỗi section
                 ...
                 return (
                   <React.Fragment key={sectionName}>
                     ...
                     {/* ✅ HÀNG "HỒ SƠ KHÁC" CUỐI MỖI SECTION */}
                         ...
                         <div className="flex flex-col gap-1.5 items-end">
                           {extraFiles.map((file, index) => (
                             ...
                             >
                               ...
                               <button
                                 onClick={() =>
                                   handleRemoveFile(extraId, index)
                                 }
                                 className="p-1 text-red-500 hover:text-red-700 hover:bg-red-100 rounded opacity-60 group-hover:opacity-100 transition-all font-bold"
                                 title="Xóa file này"
                               >
